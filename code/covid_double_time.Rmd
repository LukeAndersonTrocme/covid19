...
double_times$country_region <- 
  factor(double_times$country_region, levels = levels(covid$country_region))
...
mob_lab <-
  ...
  inner_join(. , subset %>% dplyr::select(country_region, date, from_shutdown),
...
ggplot(subset %>%
  ...
  scale_x_continuous(trans = "log2",
                     breaks = c(2,4, 7,14,28, 56, 112, 224),
                     labels = c("2 days", "4 days", "1 week", "2 weeks", "1 month", "2 months", "4 months", "8 months"),
                     limits = c(1,max(covid$doubling_time))) +
  geom_smooth(mehtod = "locfit",size = 1.5, se = F) + 
  geom_text_repel(data = label2,
                  aes(x = .fitted, 
                      y = total_cases / total_tests * 1000,
  ...
  theme(panel.grid.major.x = element_blank(),
        ...
        axis.text.x = element_text(size = 15, angle = 70, vjust = 0.5),
        axis.title = element_text(size = 15))
```
...
dual_axes <-
  ggplot(subset,
         aes(x = date,
             group = country_region)) +
  geom_smooth(aes(y = residential_percent_change_from_baseline,
                  color = "Residential % change from baseline"),
              method = "loess", se = FALSE, size = 1) +
  geom_step(aes(y = StringencyIndex, color = "Stringency Index"), size = 1.5) +
  geom_line(aes(y = log2(.fitted) * 10, color = "Doubling Time"), size = 1.5) +
  scale_y_continuous(sec.axis = sec_axis(~./10, name = "Doubling Time (Log 2)")) +
  scale_x_date(date_breaks = "1 month", date_labels = "%b")+
...
prov$prabb <- reorder(prov$prabb, prov$max_case)
