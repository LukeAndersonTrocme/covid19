...
covid %>% filter(country_region == "New Zealand")
```
...
double_times$country_region <- 
  factor(double_times$country_region, levels = max_cases$country_region)
...
lr <-
  ...
  geom_line(data = smooth_rate,
            aes(x = date,
                y = .fitted)) +
  geom_point(data = smooth_rate %>%
               filter(from_shutdown == 0),
             aes(x = date,
                 y = .fitted), 
             size = 3,
