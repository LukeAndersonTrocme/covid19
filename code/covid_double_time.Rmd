...
doubling_moments <-
  ...
  dplyr::select(country_region, from_shutdown,doubling_day, intersects) %>%
  unique() %>%
  right_join(.,
             mobility_data,
             by = c("country_region","from_shutdown")) %>%
  filter(doubling_day == T) %>%
  arrange(country_region, date) %>%
  mutate(lag_from_shutdown = lag(from_shutdown),
         lag_days_elapsed = lag(days_elapsed),
         lag_cu_cases = lag(cu_cases),
         lag_doubling_time = lag(doubling_time),
         double_day = round(intersects - lag(intersects)),
         lag_date = lag(date))
...
cu<-
  ...
  geom_segment(data = doubling_moments %>% filter(doubling_day == T),
               mapping = aes(x = lag_days_elapsed,
                             xend = days_elapsed,
                             y = cu_cases,
                             yend = cu_cases),
               color = "black",
  ...
  geom_text(data = doubling_moments %>% filter(doubling_day == T),
            mapping = aes(x = days_elapsed + 5,
                          y = cu_cases / 2,
                          label = paste(double_day,"days")),
            color = "black",
            fontface = "bold",
            size = 2,
            nudge_y = log2(2)) +
...
lr <-
  ...
  scale_y_continuous(trans = reverselog_trans(2), 
                     breaks = c(2,4, 7,14,28, 56),
                     labels = c("2 days", "4 days", "1 week", "2 weeks", "1 month", "2 months")) +
  ylab("Doubling Time\n(log2 scale)")+
  theme_minimal() +
  theme(axis.line.x = element_blank(),
        axis.text.x = element_blank(),
