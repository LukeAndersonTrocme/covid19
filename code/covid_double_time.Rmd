...
df2<-
  ...
  group_by(country_region) %>%
  arrange(date) %>%
  filter(cu_cases > 10, country_region == "Singapore") %>%  
  mutate(cases_logratio = tsibble::difference(log(cu_cases)),
         doubling_time = log(2) / cases_logratio,
         days_elapsed = as.numeric(date - min(date)))
...
df2 %>%
group_by(consec = {consec = rle(doubling_time < 14); # find consec days
rep(seq_along(consec$lengths),
    consec$lengths)}) %>%
 mutate(Consec_days = ifelse(doubling_time <= 14, NA, seq_along(consec))) %>%
  filter(Consec_days > 3,)
 ungroup() %>% 
 select(-c(consec,cname,iso3,pop_2018, cases))
```
...
sp <-
  ...
  mutate(cases_logratio = tsibble::difference(log(cu_cases)),
         ...
         total_death = max(cu_deaths),
         max_doub = max(doubling_time, na.rm = T),
         total_time = max(days_elapsed),
         ff = ifelse(country_region %in% case_countries,
                     "bold", "plain")) %>%
  group_by(country_region,
           consec = {consec = rle(doubling_time > 14); 
  rep(seq_along(consec$lengths), # find consec days
      consec$lengths)}) %>%
  mutate(Consec_days = ifelse(doubling_time >= 14, NA, seq_along(consec))) %>%
  group_by(country_region) %>%
  mutate(max_streak = max(Consec_days)) %>%
  filter(doubling_time != Inf,
         days_elapsed <= 80,
         max_case > 1000,
         max_streak > 14 | total_time > 75)
...
ggplot(dc,
  ...
  geom_point(data = dc %>% 
             ...
             size = 1,
             shape = 2) +
labs(x = "Time to reach a two week doubling time",
...
spp<-
  ...
  geom_rect(data=NULL,aes(xmin=-Inf,xmax=Inf,ymin=14,ymax=0),
                    fill="tomato")+
  geom_hline(yintercept = 14, linetype = 2, color = "grey50") +
  geom_smooth(data = sp  %>%
                filter(!country_region %in% case_countries),
              method = "loess", se = FALSE, color = "grey70", size = 0.1) + 
  geom_smooth(data = sp  %>%
                filter(country_region %in% case_countries),
  ...
  guides(size = F) +
  labs(x = "Days since 10th reported case",
       y = "Doubling time\n(log2 scale)")+
  theme_minimal() +
...
ggsave(paste0(covidFigPath, "time_to.jpg"), height = 8, width = 16)
```
